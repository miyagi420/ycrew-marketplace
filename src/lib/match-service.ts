...
import { USE_MATCH_EDGE_FUNCTIONS } from './env';
...
export async function getCandidatesForJob(jobId: string): Promise<Candidate[]> {
  ...
  const appMap = new Map<string, string>(
    (apps ?? []).map((a) => [a.crew_user_id, a.status]),
  );
...
async function rankJobsClient(uid: string): Promise<RankedJob[]> {
  ...
  const vmap = await vesselTypeMap(
    (jobs ?? []).map((j) => j.vessel_id).filter((v): v is string => !!v),
  );
...
async function rankCandidatesClient(
  jobId: string,
